...
class EPW(QMainWindow):
    ...
    def UI(self):
        ...
        self.info = QPushButton("Edit Prodcut")
        ...
        self.Fill()
    def Fill(self):
        data = PRR.Select("*", "Products", f"ID={self.__id}")
        self.name_input.setText(data[0][1])
        sc_name = str(PRR.Select("CompanyName", "Suppliers", f"ID={data[0][2]}")[0][0])
        self.sc_input.setCurrentText(str(data[0][2])+" - "+str(sc_name))
        self.unit_input.setText(data[0][3])
        self.amount_input.setText(str(data[0][4]))
        self.bp_input.setText(str(data[0][5]))
        self.sp_input.setText(str(data[0][6]))
    ...
    def Submit(self):
        ...
        if name.strip() == "" and sc.strip() == "" and unit.strip() == "" and amount.strip() == "" and bp.strip() == "" and sp.strip() == "":
            ...
            PRR.Update("Products", f"Name='{name}', SC={sc}, Unit='{unit}', Amount={amount}, BuyPrice={bp}, SalePrice={sp}", f"ID={self.__id}")
            self.__cf()

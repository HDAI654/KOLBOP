...
from PL.Services.AScW_win import AScW
from PL.Services.ES_sc_win import ES_sc_W

class ScW(QMainWindow):
    def __init__(self):
        super().__init__()
        self.search_combo_index = 0
        self.search_input_text = ""
        self.services_HEADS = ["ID", "Explanation", "Unit", "Price", "Delete Tool", "Show/Edit"]
        self.StackUI()
        self.ToolbarUI()
        self.TableUI()
    def StackUI(self):
        ...
        self.add_frame = AScW(self.asw_close)
    ...
    def TableUI(self):
        ...
        self.services_table_widget = QTableWidget()
        self.services_table_widget.setColumnCount(len(self.services_HEADS))
        self.services_table_widget.setHorizontalHeaderLabels(self.services_HEADS)
        self.services_table_widget.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.services_table_widget.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.table_frame.setCentralWidget(self.services_table_widget)

        # Do search
        self.Search()
    def ToolbarUI(self):
        # Toolbar
        self.toolbar = QToolBar("Services Page Tools")
        ...
        # Icon
        self.ServicesIcon = QLabel()
        self.ServicesIcon.setPixmap(QPixmap("Assets/Images/Services.png"))
        self.ServicesIcon.setAlignment(Qt.AlignCenter)
        self.toolbar.addWidget(self.ServicesIcon)
        self.toolbar.addSeparator()

        # Actions
        self.delete_all_action = QAction(QIcon(QPixmap("Assets/Images/Delete All.png")), "Delete All Services (Ctrl+Alt+D)", self)
        ...
        self.add_action = QAction(QIcon(QPixmap("Assets/Images/Add2.png")), "Add New Service (Ctrl+N)", self)
        ...
        self.search_combo.addItems(["All", "ID", "Explanation", "Unit", "Price"])
    ...
    def Search(self):
        ...
        # Get Table Data
        table_data = list(PRR.Select("*", "Services", "true"))
        ...
        if table_data != None:
            ...
            if self.search_input.text().strip() == "" :
                self.FillTable(data=list(PRR.Select("*", "Services", "true")))
    def FillTable(self, data):
        if data != None :
            self.services_table_widget.setRowCount(len(data))

            # Set Data
            for row in range(len(data)):
                for column in range(len(data[row])):
                    self.services_table_widget.setItem(row, column, QTableWidgetItem(str(data[row][column])))

            # Set Delete Buttons
            for row_index, row_data in enumerate(data):
                button = QPushButton("Delete")
                self.services_table_widget.setCellWidget(row_index, 4, button)
            ...
            for row_index, row_data in enumerate(data):
                ...
                self.services_table_widget.setCellWidget(row_index, 5, button)
    ...
    def DeleteSupplier(self, row, id):
        PRR.Delete("ُServices", f"ID={id}")
        self.Refresh()
    def ShowEdit(self, row, id):
        self.stacked_widget.removeWidget(self.edit_show_frame)
        self.edit_show_frame = ES_sc_W(int(id), self.shw_close)

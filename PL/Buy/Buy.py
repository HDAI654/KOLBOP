...
from PL.Buy.AB_win import ABW
from PL.Buy.SE_win import SEB
...
class BW(QMainWindow):
    ...
    def StackUI(self):
        ...
        self.add_frame = ABW(self.ABW_close)
    ...
    def ToolbarUI(self):
        # Toolbar
        self.toolbar = QToolBar("Buy Page Tools")
        ...
        # Icon
        self.Icon = QLabel()
        self.Icon.setPixmap(QPixmap("Assets/Images/buy.png"))
    ...
    def Search(self):
        ...
        # Get Table Data
        table_data = list(PRR.Select('*', 'Buy', 'true'))
        ...
        if table_data != None:
            ...
            if self.search_input.text().strip() == "" :
                self.FillTable(data=list(PRR.Select('*', 'Buy', 'true')))
    ...
    def FillTable(self, data):
        if data != None :
            ...
            for row_index, row_data in enumerate(data):
                ...
                button.clicked.connect(lambda checked, row=row_index, id=row_data[0]: self.DeleteBuy(row, id))
    ...
    def DeleteBuy(self, row, id):
        # Delete Buy Products
        PRR.Delete("Buy_Products", f"BC={int(id)}")
        # Delete Buy Services
        PRR.Delete("Buy_Services", f"BC={int(id)}")
        # Delete Buy Suppliers
        PRR.Delete("Buy_Suppliers", f"BC={int(id)}")
        # Delete Buy
        PRR.Delete("Buy", f"ID={int(id)}")
        # Refresh
        self.Refresh()
    
    def ShowEdit(self, row, id):
        self.stacked_widget.removeWidget(self.edit_show_frame)
        self.edit_show_frame = SEB(self.shw_close, int(id))
        self.stacked_widget.addWidget(self.edit_show_frame)

...
class SEB(QMainWindow):
    ...
    def UI(self):
        ...
        self.products_page = PSTW("Buy", data=self.Data["Buy_Products"])
        self.services_page = SSTW(data=self.Data["Buy_Services"])
        self.suppliers_page = SPSTW(data=self.Data["Buy_Suppliers"])
    ...
    def data(self):
        ...
        # Buy
        D["Buy"] =  PRR.Select("*", "Buy", f"ID={self.id}")[0]

        # Buy Products
        p = PRR.Select("PC, Name, Unit, Amount, Price", "Buy_products", f"BC={self.id}")
        np = []
        for ap in p:
            np.append(list(ap))
        D["Buy_Products"] = np
        ...
        # Buy Services
        bsf = PRR.Select("*", "Buy_Services", f"BC={self.id}")
        bs=[]
        for s in bsf:
            bsr = []
            # service code
        ...
        # Buy Suppliers
        bspf = PRR.Select("*", "Buy_Suppliers", f"BC={self.id}")
        bsp = []
        for sp in bspf:
            spr = []
            # supplier code
    ...
    def Submit(self):
        ...
        if tp.strip() == "" or date_time.strip() == "" or self.products_page.GetData() == []:
            ...
            # Buy
            PRR.Update("Buy", f"TotalPrice={tp}, Date='{date_time}'",f"ID={self.id}")

            
            # Product
            #delete old
            PRR.Delete("Buy_Products", f"BC={self.id}")
            #add new
            for p in self.products_page.GetData():
                PRR.Insert("Buy_Products", "PC, Name, Unit, Amount, Price, BC", f"{p[0]}, '{p[1]}', '{p[2]}', '{p[3]}', {p[4]}, {self.id}")
            
            # Sevice
            #delete old
            PRR.Delete("Buy_Services", f"BC={self.id}")
            #add new
            for s in self.services_page.GetData():
                PRR.Insert("Buy_Services", "BC, SVC, Explanation, Unit, Price", f"{self.id}, {s[0]}, '{s[1]}', '{s[2]}', {s[3]}")

            # Suppliers
            #delete old
            PRR.Delete("Buy_Suppliers", f"BC={self.id}")
            #add new
            for sp in self.suppliers_page.GetData():
                PRR.Insert("Buy_Suppliers", "BC, SC, Supplier_Name", f"{self.id}, {sp[0]}, '{sp[1]}'")

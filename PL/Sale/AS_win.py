...
class ASW(QMainWindow):
    ...
    def UI(self):
        ...
        self.products_page = PSTW("Sale", Page_Name="Sale Products")
        self.services_page = SSTW(Page_Name="Sale Services")
        self.clients_page = CSTW()
        ...
        self.notebook.addTab(self.clients_page, "Clients")
    ...
    def fields_page_UI(self):
        ...
        self.info = QPushButton("Add Sale")
    ...
    def Submit(self):
        ...
        if tp.strip() == "" or date_time.strip() == "" or self.products_page.GetData() == []:
            ...
            # Sale
            PRR.Insert("Sale", "TotalPrice, Date", f"{tp}, '{date_time}'")
            bI = int(PRR.Select("ID", "Sale", "true")[-1][0])

            # Product
            for p in self.products_page.GetData():
                PRR.Insert("Sale_Products", "PC, Name, Unit, Amount, Price, SLC", f"{p[0]}, '{p[1]}', '{p[2]}', '{p[3]}', {p[4]}, {bI}")

            # Sevice
            for s in self.services_page.GetData():
                PRR.Insert("Sale_Services", "SLC, SVC, Explanation, Unit, Price", f"{bI}, {s[0]}, '{s[1]}', '{s[2]}', {s[3]}")

            # Clients
            for sp in self.clients_page.GetData():
                PRR.Insert("Sale_Clients", "SLC, CLC, Client_Name", f"{bI}, {sp[0]}, '{sp[1]}'")

            self.products_page.clear_table()
            self.services_page.clear_table()
            self.clients_page.clear_table()
            v = str(datetime.date.today()).split("-")
